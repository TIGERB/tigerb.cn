...
那么我们就假设：**按照原`TCMalloc`的思想，把`mcache`绑定系统线程`M`上**。

要论证这个假设对于Go语言而言会存在什么问题，需要先来简单看看「Go的调度模型`GMP`」

### Go的调度模型`GMP`

首先直接上入门级「Go的调度模型`GMP`」架构图：
...
- `G`：代表结构体`g`，全称`Goroutine`，这个结构体就是大家熟知的`协程`，简单理解其实就是这个结构体唯一绑定了一个有着被并发执行需求的函数。
...
当`G`执行一个I/O操作的系统调用时，比如`read`、`write`，因为系统调用过程中的阻塞(不在本文范畴，后续文章详解)问题，会发生如下操作：
